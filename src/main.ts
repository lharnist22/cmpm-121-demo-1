...
upgrade1.textContent = "Preworkout";
...
upgrade1.addEventListener("click", () => {
  if (counter >= 10 + upgrade1c * 1.15) {
    ...
    upgrade1c += 1;
    counterDisplay.textContent = `Reps: ${counter}`; // Updating counter to appear properly
    dupgrade1c.textContent = `Preworkout: ${upgrade1c}`; // Updating num of upgrades
  }
});

const upgrade2 = document.createElement("button") as HTMLButtonElement;
upgrade2.textContent = "Creatine";
...
upgrade2.addEventListener("click", () => {
  if (counter >= 100) {
    ...
    upgrade2c += 1;
    counterDisplay.textContent = `Reps: ${counter}`; // Updating counter to appear properly
    dupgrade2c.textContent = `Creatine: ${upgrade2c}`; // Updating num of upgrades
  }
});

const upgrade3 = document.createElement("button") as HTMLButtonElement;
upgrade3.textContent = "Tren";
...
upgrade3.addEventListener("click", () => {
  if (counter >= 1000) {
    ...
    upgrade3c += 1;
    counterDisplay.textContent = `Reps: ${counter}`; // Updating counter to appear properly
    dupgrade3c.textContent = `Tren: ${upgrade3c}`; // Updating num of upgrades
  }
});
